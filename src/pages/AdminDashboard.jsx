...
export default function AdminDashboard() {
  ...
  const handleAccountSubmit = async (e) => {
    ...
    const endpoint = accountData.role === 'employee'
      ? `${import.meta.env.VITE_API_URL}/api/auth/employe`
    ...
    try {
      ...
      if (response.ok) {
        setAccountMessage({ type: 'success', text: data.message || 'Account created successfully!' });
        setAccountData({
          username: '',
          password: '',
          role: 'user'
        ...
        setAccountMessage({ type: 'error', text: data.message || 'Failed to create account.' });
      }
    } catch (err) {
  ...
  return (
    <div className="gckey-page" style={{ backgroundColor: '#fff', minHeight: '100vh', fontFamily: 'Noto Sans, sans-serif' }}>
      <SEO
        title="Admin Dashboard - Immigration, Refugees and Citizenship Canada"
        description="Admin dashboard for managing visas and user accounts."
        keywords="admin dashboard, manage visas, create account"
      />
              ...
              <>
                ...
                <div className="col-md-6 col-sm-12" style={{ marginBottom: '20px', display: 'flex' }}>
                  ...
                  >
                    ...
                    <p style={{ color: '#333', fontSize: '16px' }}>Create new admin or user accounts.</p>
                  </div>
                </div>
              </>
            )}
